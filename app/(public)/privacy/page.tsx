...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const content = await getSiteContentBySectionAdmin('privacy');
  const c = contentMap(content);
  return {
    title: pickSiteContent(c, 'page_privacy_title', locale, m.page_privacy_title),
    description: pickSiteContent(c, 'page_privacy_meta', locale, m.page_privacy_meta),
  };
}
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-14 sm:py-20">
        <article className="rounded-2xl border bg-card/80 backdrop-blur-sm shadow-sm p-8 sm:p-10">
          <h1 className="text-3xl sm:text-4xl font-bold tracking-tight text-foreground">
            {tx('page_privacy_title', m.page_privacy_title)}
          </h1>
          <p className="mt-2 text-sm text-muted-foreground">
            {tx('page_privacy_updated', m.page_privacy_updated)}
          </p>
          <div className="mt-8 space-y-5 text-muted-foreground leading-relaxed">
            <p>{tx('page_privacy_p1', m.page_privacy_p1)}</p>
            <p>{tx('page_privacy_p2', m.page_privacy_p2)}</p>
          </div>
          <div className="mt-10">
